...
% Assuming that allFeatures and allLabels have been populated with the extracted features and corresponding labels

% Perform PCA on the feature matrix
[coeff, score, latent, tsquared, explained, mu] = pca(allFeatures);

% Choose the number of principal components to retain based on explained variance
explainedVarianceThreshold = 95; % Keep enough components to explain 95% of the variance
...
X_train = reducedFeatures(training(cv), :);
y_train = allLabels(training(cv));
X_test = reducedFeatures(test(cv), :);
y_test = allLabels(test(cv));

% Define the template tree
...
Mdl = fitcensemble(X_train, y_train, 'Method', 'Bag', 'Learners', t, ...
...
% Get feature importance (after PCA, these correspond to principal components)
...
title('Predictor Importance Estimates after PCA');
...
cv = cvpartition(length(allLabels), 'HoldOut', 0.1);
idxTrain = cv.training;
idxTest = cv.test;

% Training data
trainFeatures = allFeatures(idxTrain, :);
trainLabels = allLabels(idxTrain);

% Test data
testFeatures = allFeatures(idxTest, :);
testLabels = allLabels(idxTest);

% Train a Random Forest classifier using TreeBagger
nTrees = 500;  % Number of trees in the forest
inBagFraction = 0.6;  % Fraction of data used for training each tree
...
figure;
heatmap(cellTypes, cellTypes, confMat);
